...
"""
Test case 4: Deletion and Addition of the same VRF and testing data path
"""
...
def test_ping_mulitimes_add_and_delete_samevrf(topology, configuration, step):
    """
    ...
    for x in range(0, COUNT):
        ...
        for sw in [sw1, sw2]:
            ...
            for vrf_name in vrfs:
                ...
                if retval:
                    ...
                    step("vrf {0} successfully deleted".format(vrf_name))
        ...
        step('###### Step 3 - Test the ping through VRF default ######')
        ping = hs1.libs.ping.ping(1, HS2)
        assert ping['transmitted'] == ping['received'] == 1

        step('###### Step 4 - Reconfigure the same VRF ######')
        vrfs = [vrf_red]
        for sw in [sw1, sw2]:
            for vrf_name in vrfs:
                add_vrf(sw, vrf_name)
        ...
        sleep(2)
        step('###### Step 5 - Test the ping through VRF RED ######')
        ping = hs1.libs.ping.ping(1, HS2)
        assert ping['transmitted'] == ping['received'] == 1

    step('##### TEST CASE vrf.04 PASSED #####')

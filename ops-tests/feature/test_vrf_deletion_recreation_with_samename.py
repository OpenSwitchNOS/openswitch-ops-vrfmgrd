...
"""
Test case 11: VRF deletion and Recreation with same name -- clean ups
"""
...
def test_vrf_delete_recreate_with_samename(topology, configuration, step):
    ...
    for x in range(0, COUNT):
        ...
        for vrf_name in vrfs:
            ...
            if retval:
                ...
                step('##### vrf {0} successfully'
                     ' deleted #####'.format(vrf_name))
        ...
        sleep(2)
        step('###### Step 4 - ReAdd the VRFs with samename ######')
        for vrf_name in vrfs:
            add_vrf(sw1, vrf_name)
        ...
        for vrf_name in vrfs:
            retval = validate_configured_vrf(output, vrf_name)
            if retval:
                step("##### vrf {0} exist #####".format(vrf_name))
            else:
                assert False, "vrf {0} creation failed".format(vrf_name)
        ...
        for portidx in sw1.ports.values():
            if portidx is '1' or portidx is '2':
                if 'vrf1' in result.keys():
                    result['vrf1'].append(portidx)
                else:
                    result['vrf1'] = []
                    result['vrf1'].append(portidx)
            else:
                temp = 'vrf'+str(int(portidx) - 1)
                result[temp] = portidx

        for vrf_name in result.keys():
            output = get_mapped_interface_to_vrf(sw1, vrf_name)
            for portidx in result[vrf_name]:
                if portidx in output:
                    ...
                    assert False, "Port {0} "
                    "not Mapped to {1}".format(portidx, vrf_name)
    ...
    sleep(2)
    step('###### Step 7 - Final Validation to the ping through vrf1'
         ' should not drop any ping packets ######')
    ping = hs1.libs.ping.ping(1, HS2)
    assert ping['transmitted'] == ping['received'] == 1

    step('##### TEST CASE vrf.11 PASSED')

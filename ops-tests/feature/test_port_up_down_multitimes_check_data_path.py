...
"""
Test case 5: Make interface down/up while doing PING
"""
...
from .vrf_configs import wait_until_interface_up, change_interface_status
...
def test_ping_while_changing_interface_status(topology, configuration, step):
    """
    ...
    for x in range(0, COUNT):
        cmd = 'ping 10.0.3.1 > /tmp/ping.txt 2>&1 &'
        hs1(cmd.format(**locals()), shell='bash')
        step('###### Step 2 - Make interface down while ping'
             'is going through ######')
        change_interface_status(sw1, p11, "down")
        sleep(5)
        ...
        if ping:
            if ping.split(' ')[0] != ping.split(' ')[3]:
                step('###### Step 3 - ping gets breaked ######')
            else:
                assert False, "Ping gets success even interface is down"
        else:
            assert False, "Unable to get ping statistics"

        step('###### Step 3 - Make interface up ######')
        change_interface_status(sw1, p11, "up")
        # Wait until interfaces are up
        for switch, portlbl in [(sw1, p11)]:
            wait_until_interface_up(switch, portlbl)
        ...
        if ping:
            if ping.split(' ')[0] == ping.split(' ')[3]:
                step('###### Step 4 - ping gets success ######')
            else:
                assert False, "Ping gets failed even interface is up"
        else:
            assert False, "Unable to get ping statistics"
    step('##### TEST CASE vrf.08 PASSED')

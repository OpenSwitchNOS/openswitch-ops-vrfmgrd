...
#     else:
#         for j in range(1, 5):
#             interface = '{i}.{j}'.format(i=i, j=j)
#             vrfs.append('vrf' + interface)
vrfs = []
for i in range(1, 65):
        vrfs.append('vrf'+str(i))
...
def test_testing_vrf_scale(topology, configuration, step):
    ...
    with sw1.libs.vtysh.Configure() as ctx:
        ctx.ip_route('10.0.3.0/24', '10.0.2.2', vrf_name=vrf_1)

    with sw2.libs.vtysh.Configure() as ctx:
        ctx.ip_route('10.0.1.0/24', '10.0.2.1', vrf_name=vrf_1)
    ...
    for x in range(0, COUNT):
        ...
        for vrf_name in vrfs:
            ...
            if retval:
                ...
                print("vrf {0} does not exist".format(vrf_name))

        sleep(2)

        step('###### Step 3 - ReAdd the 64 VRFs ######')
        for vrf_name in vrfs:
            add_vrf(sw1, vrf_name)
        ...
        # Mapping ports to vrf in Switch 1
        step('###### Step 4 - Attaching interfaces to few vrfs  ######')
        ...
        sleep(5)
        step('###### Step 5 - Validate the ping through VRF RED'

...
## Contents
   ...
   - [Verify the interface is moved out of the VRF](#test_interface_moved_out_of_vrf)
   ...
   ## Verify the interface is moved out of the VRF
   #### Objective
   This test case confirms that
        1- Add a port to VRF and verify the Nexthop ping from that VRF
        2- Move the attacted interface from that VRF to another VRF
        3- Verify the Nexthop ping is succeding for the new VRF and ping should fail for older VRF
   #### Requirements
   - Physical Switch/Switch Test setup
   - **FT File**:
   #### Setup
   ...
   ```
   #### Description
   1. Setup the topology as shown, and map the supported interface level to each configured vrf
   ...
   13. Configure IP address for all the interfaces moved to vrf_default from vrf_red
   14. Add a static route from sw1 to hs2 via sw2 and sw2 to hs1 via sw1
   15. Initiate host to host ping to test the ping through vrf_default
   16. Once again attach all the same interfaces to vrf_red
   17. Use CLI command to verify the mapped interfaces are moved to vrf_red
   18. Configure ip address
   19. Repeat the steps 10,11,12,13,14 and 15 for 'n' number of times
   20. Initiate host to host ping to test the ping through vrf_red

...
##############################################################################
# Name:     test_vrf-based-ping-between-switches-for-multilinks
#           -map-to-singlevrf.py
#
# Objective: Add 2 interfaces to same VRF and check ping via 1 to 2 is going
#            through
#
# Topology:  2 switches (DUT running Halon) connected by 8 interfaces
#
##############################################################################
...
SW1_INTF2_IPV4_ADDR = "20.0.0.1/24"
SW1_INTF3_IPV4_ADDR = "30.0.0.1/24"
SW1_INTF4_IPV4_ADDR = "40.0.0.1/24"
SW1_INTF5_IPV4_ADDR = "50.0.0.1/24"
SW1_INTF6_IPV4_ADDR = "60.0.0.1/24"
SW1_INTF7_IPV4_ADDR = "70.0.0.1/24"
SW1_INTF8_IPV4_ADDR = "80.0.0.1/24"
...
SW2_INTF2_IPV4_ADDR = "20.0.0.2/24"
SW2_INTF3_IPV4_ADDR = "30.0.0.2/24"
SW2_INTF4_IPV4_ADDR = "40.0.0.2/24"
SW2_INTF5_IPV4_ADDR = "50.0.0.2/24"
SW2_INTF6_IPV4_ADDR = "60.0.0.2/24"
SW2_INTF7_IPV4_ADDR = "70.0.0.2/24"
SW2_INTF8_IPV4_ADDR = "80.0.0.2/24"
...
# Configure ip address of default vrf links in Switch 1
    configure_ipaddress(sw1, SW1_INTF1, SW1_INTF1_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF2, SW1_INTF2_IPV4_ADDR)

# Configure ip address of vrf links in Switch 1
    configure_ipaddress(sw1, SW1_INTF3, SW1_INTF3_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF4, SW1_INTF4_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF5, SW1_INTF5_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF6, SW1_INTF6_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF7, SW1_INTF7_IPV4_ADDR)
    configure_ipaddress(sw1, SW1_INTF8, SW1_INTF8_IPV4_ADDR)

# Configure ip address of default vrf links in Switch 2
    configure_ipaddress(sw2, SW2_INTF1, SW2_INTF1_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF2, SW2_INTF2_IPV4_ADDR)

# Configure ip address of vrf links in Switch 2
    configure_ipaddress(sw2, SW2_INTF3, SW2_INTF3_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF4, SW2_INTF4_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF5, SW2_INTF5_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF6, SW2_INTF6_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF7, SW2_INTF7_IPV4_ADDR)
    configure_ipaddress(sw2, SW2_INTF8, SW2_INTF8_IPV4_ADDR)
...
def test_vrf_based_ping_multilink_in_samevrf(topology, configuration, step):
    """
    Test case:
       Add 2 interfaces to same VRF and check ping via 1 to 2 is going through
    """
    sw1 = topology.get("sw1")
    sw2 = topology.get("sw2")
...
# Validate Mapped ports in Switch 1 vrf's
    ...
    for interface in ports_vrf_default_sw1:
        if interface in output:
            step("Port {0} Mapped to {1}".format(interface, vrf_default))
        else:
            assert False, "Port {0} "
    ...
    for interface in ports_vrf_red_sw1:
        if interface in output:
            step("Port {0} Mapped to {1}".format(interface, vrf_red))
        else:
            assert False, "Port {0} "
    ...
    for interface in ports_vrf_green_sw1:
        if interface in output:
            step("Port {0} Mapped to {1}".format(interface, vrf_green))
        else:
            assert False, "Port {0} "
    ...
    for interface in ports_vrf_blue_sw1:
        if interface in output:
            step("Port {0} Mapped to {1}".format(interface, vrf_blue))
        else:
            assert False, "Port {0} "
...
# VRF based ping for remote link's including default vrf
    ping = sw1.libs.vtysh.ping('10.0.0.2', count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('20.0.0.2', count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('30.0.0.2', vrf=vrf_red, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('40.0.0.2', vrf=vrf_red, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('50.0.0.2', vrf=vrf_green, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('60.0.0.2', vrf=vrf_green, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('70.0.0.2', vrf=vrf_blue, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('80.0.0.2', vrf=vrf_blue, count=1)
    assert ping['transmitted'] == ping['received'] == 1

    step('TEST CASE vrf.03 PASSED')

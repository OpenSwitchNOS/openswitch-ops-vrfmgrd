...
##############################################################################
# Name:      test_vrf-based-ping-between-switches-for-samevrf.py
#
# Objective:
...
#       mapped to different Vrfs and check if it is pinging fine with VRF
#
...
# VRF based ping for remote link's including default vrf
    ...
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('10.0.0.2', vrf=vrf_green, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping('10.0.0.2', vrf=vrf_blue, count=1)
    assert ping['transmitted'] == ping['received'] == 1

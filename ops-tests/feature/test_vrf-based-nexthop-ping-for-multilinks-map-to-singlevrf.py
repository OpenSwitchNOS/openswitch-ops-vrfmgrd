...
"""
Test case 3: Add 2 interfaces to same VRF and verify the Nexthop ping
...
def test_vrf_based_ping_multilink_in_samevrf(topology, configuration, step):
    """
    Test case:
       Add 2 interfaces to same VRF and verify the Nexthop ping
       for all the interfaces
    """
    sw1 = topology.get("sw1")
    sw2 = topology.get("sw2")
...
# VRF based ping for remote link's including default vrf
    ping = sw1.libs.vtysh.ping(SW2_INTF1_IPV4_ADDR.split('/')[0], count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF2_IPV4_ADDR.split('/')[0], count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF3_IPV4_ADDR.split('/')[0],
                               vrf=vrf_red, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF4_IPV4_ADDR.split('/')[0],
                               vrf=vrf_red, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF5_IPV4_ADDR.split('/')[0],
                               vrf=vrf_green, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF6_IPV4_ADDR.split('/')[0],
                               vrf=vrf_green, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF7_IPV4_ADDR.split('/')[0],
                               vrf=vrf_blue, count=1)
    assert ping['transmitted'] == ping['received'] == 1
    ping = sw1.libs.vtysh.ping(SW2_INTF8_IPV4_ADDR.split('/')[0],
                               vrf=vrf_blue, count=1)
    assert ping['transmitted'] == ping['received'] == 1
